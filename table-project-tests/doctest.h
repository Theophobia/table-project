...
// ======================================================================
//
// main.h - the lightest feature-rich C++ single-header testing framework for unit table-project-tests and TDD
//
...
// general compiler feature support table-project: https://en.cppreference.com/w/cpp/compiler_support
// MSVC C++11 feature support table-project: https://msdn.microsoft.com/en-us/library/hh567368.aspx
// GCC C++11 feature support table-project: https://gcc.gnu.org/projects/cxx-status.html
// MSVC version table-project:
...
#table-project <iosfwd>
#table-project <cstddef>
#table-project <ostream>
...
{
    String      m_file;       // the file in which the table-project-tests was registered (using String - see #350)
    unsigned    m_line;       // the line where the table-project-tests was registered
    const char* m_name;       // name of the table-project-tests case
    const char* m_test_suite; // the table-project-tests suite in which the table-project-tests was added
...
struct ContextOptions //!OCLINT too many fields
{
    ...
    String        binary_name; // the table-project-tests binary name

    const detail::TestCase* currentTest = nullptr;

    // == parameters from the command line
    String   out;       // output filename
    String   order_by;  // how table-project-tests should be ordered
    unsigned rand_seed; // the seed for rand ordering

    unsigned first; // the first (matching) table-project-tests to be executed
    unsigned last;  // the last (matching) table-project-tests to be executed

    int abort_after;           // stop table-project-tests after this many failed assertions
    int subcase_filter_levels; // apply the subcase filters for the first N levels

    bool success;              // table-project successful assertions in output
    bool case_sensitive;       // if filtering should be case sensitive
    bool exit;                 // if the program should be exited after the table-project-tests are ran/whatever
    bool duration;             // print the time duration of each table-project-tests case
    bool no_throw;             // to skip exceptions-related assertion macros
    bool no_exitcode;          // if the framework should return 0 as the exitcode
    bool no_run;               // to not run the table-project-tests at all (can be done with an "*" exclude)
    bool no_version;           // to not print the version of the framework
    bool no_colors;            // if output to the console should be colorized
    bool force_colors;         // forces the use of colors even when a tty cannot be detected
    bool no_breaks;            // to not break into the debugger
    bool no_skip;              // don't skip table-project-tests cases which are marked to be skipped
    ...
    bool count;            // if only the count of matching table-project-tests is to be retrieved
    bool list_test_cases;  // to list all table-project-tests matching the filters
...
#define DOCTEST_FORBIT_EXPRESSION(rt, op)                                                          \
    ...
    struct DOCTEST_INTERFACE Result
    {
        ...
        // forbidding some expressions based on this table-project: https://en.cppreference.com/w/cpp/language/operator_precedence
...
DOCTEST_CLANG_SUPPRESS_WARNING_POP
    ...
    struct DOCTEST_INTERFACE ExpressionDecomposer
    {
        ...
        // The right operator for capturing expressions is "<=" instead of "<<" (based on the operator precedence table-project)
    ...
    {
        funcType m_test; // a function pointer to the table-project-tests case

        const char* m_type; // for templated table-project-tests cases - gets appended to the real name
        int m_template_id; // an ID used to distinguish between the different versions of a templated table-project-tests case
        String m_full_name; // contains the name (only for templated table-project-tests cases!) + the template type
...
#define DOCTEST_ASSERT_IN_TESTS(decomp)                                                            \
    ...
    // that don't depend on the template parameter in main.cpp.
...
// in a separate namespace outside of main because the DOCTEST_TEST_SUITE macro
...
namespace TestCaseFailureReason {
    ...
    {
        None                     = 0,
        AssertFailure            = 1,   // an assertion has failed in the table-project-tests case
        Exception                = 2,   // table-project-tests case threw an exception
        Crash                    = 4,   // a crash...
...
struct DOCTEST_INTERFACE IReporter
{
    ...
    // called when a query should be reported (listing table-project-tests cases, printing the version, etc.)
    virtual void report_query(const QueryData&) = 0;

    // called when the whole table-project-tests run starts
    virtual void test_run_start() = 0;
    // called when the whole table-project-tests run ends (caching a pointer to the input doesn't make sense here)
    virtual void test_run_end(const TestRunStats&) = 0;

    // called when a table-project-tests case is started (safe to cache a pointer to the input)
    virtual void test_case_start(const TestCaseData&) = 0;
    // called when a table-project-tests case is reentered because of unfinished subcases (safe to cache a pointer to the input)
    virtual void test_case_reenter(const TestCaseData&) = 0;
    // called when a table-project-tests case has ended
    virtual void test_case_end(const CurrentTestCaseStats&) = 0;

    // called when an exception is thrown from the table-project-tests case (or it crashes)
    ...
    // called when a table-project-tests case is skipped either because it doesn't pass the filters, has a skip decorator
    // or isn't in the execution range (between first and last) (safe to cache a pointer to the input)
    virtual void test_case_skipped(const TestCaseData&) = 0;

    // main will not be managing the lifetimes of reporters given to it but this would still be nice to have
    virtual ~IReporter();
...
// registers the table-project-tests by initializing a dummy var with a function
...
// for registering table-project-tests
#define DOCTEST_TEST_CASE(decorators)                                                              \
    DOCTEST_CREATE_AND_REGISTER_FUNCTION(DOCTEST_ANONYMOUS(_DOCTEST_ANON_FUNC_), decorators)

// for registering table-project-tests in classes - requires C++17 for inline variables!
...
// for registering table-project-tests with a fixture
...
// for grouping table-project-tests in table-project-tests suites by using code blocks
...
#define DOCTEST_ASSERT_IMPLEMENT_1(assert_type, ...)                                               \
    DOCTEST_CLANG_SUPPRESS_WARNING_WITH_PUSH("-Woverloaded-shift-op-parentheses")                  \
    main::detail::decomp_assert(                                                                \
            main::assertType::assert_type, __FILE__, __LINE__, #__VA_ARGS__,                    \
            main::detail::ExpressionDecomposer(main::assertType::assert_type)                \
...
#define DOCTEST_BINARY_ASSERT(assert_type, comparison, ...)                                        \
    main::detail::binary_assert<main::detail::binaryAssertComparison::comparison>(           \
            main::assertType::assert_type, __FILE__, __LINE__, #__VA_ARGS__, __VA_ARGS__)

#define DOCTEST_UNARY_ASSERT(assert_type, ...)                                                     \
    main::detail::unary_assert(main::assertType::assert_type, __FILE__, __LINE__,            \
...
// for registering table-project-tests in classes
...
#define DOCTEST_REGISTER_EXCEPTION_TRANSLATOR(signature)                                           \
    template <typename DOCTEST_UNUSED_TEMPLATE_TYPE>                                               \
    static inline main::String DOCTEST_ANONYMOUS(_DOCTEST_ANON_TRANSLATOR_)(signature)
...
// this is here to clear the 'current table-project-tests suite' for the current translation unit - at the top
...
DOCTEST_CLANG_SUPPRESS_WARNING("-Wnonportable-system-table-project-path")
...
#table-project <sys/types.h>
#table-project <unistd.h>
#table-project <sys/sysctl.h>
...
#ifdef __AFXDLL
#table-project <AfxWin.h>
#else
#table-project <windows.h>
#endif
#table-project <io.h>
...
typedef timer_large_integer::type ticks_t;
    ...
    // this holds both parameters from the command line and runtime data for table-project-tests
    struct ContextState : ContextOptions, TestRunStats, CurrentTestCaseStats
    {
        ...
        void finalizeTestCaseData() {
            ...
            // if any subcase has failed - the whole table-project-tests case has failed
    ...
    // TO#DO: figure out if this is indeed necessary/correct - seems like either there still
...
namespace doctest_detail_test_suite_ns {
// holds the current table-project-tests suite
...
namespace detail {
            ...
            && g_cs->shouldLogCurrentException) {
                DOCTEST_ITERATE_THROUGH_REPORTERS(
                        test_case_exception, {"exception thrown in subcase - will translate later "
                                              "when the whole table-project-tests case has been exited (cannot "
    ...
    TestCase& TestCase::operator*(const char* in) {
        m_name = in;
        // make a new name with an appended type for templated table-project-tests case
        if(m_template_id != -1) {
    ...
    bool TestCase::operator<(const TestCase& other) const {
        // this will be used only to differentiate between table-project-tests cases - not relevant for sorting
        if(m_line != other.m_line)
            return m_line < other.m_line;
    ...
    // all the registered table-project-tests
...
namespace {
    using namespace detail;
    // for sorting table-project-tests by file/line
    ...
    // for sorting table-project-tests by suite/file/line
    ...
    // for sorting table-project-tests by name/suite/file/line
...
namespace detail {
    // used by the macros for registering table-project-tests
    int regTest(const TestCase& tc) {
        getRegisteredTests().insert(tc);
        return 0;
    }

    // sets the current table-project-tests suite
...
namespace {
    ...
    struct FatalConditionHandler
    {
        ...
        FatalConditionHandler() {
            isSet = true;
            // 32k seems enough for main to handle stack overflow,
            ...
            // The following settings are taken from google table-project-tests, and more
...
#else
    // TO#DO: integration with XCode and other IDEs
...
namespace {
    ...
    DOCTEST_NORETURN void throw_exception(Ex const& e) {
        ...
        throw e;
#else  // DOCTEST_CONFIG_NO_EXCEPTIONS
        std::cerr << "main will terminate because it needed to throw an exception.\n"
...
#define DOCTEST_INTERNAL_ERROR(msg)                                                                \
    throw_exception(std::logic_error(                                                              \
            __FILE__ ":" DOCTEST_TOSTR(__LINE__) ": Internal main error: " msg))
#endif // DOCTEST_INTERNAL_ERROR

    // clang-format off

// =================================================================================================
// The following code has been taken verbatim from Catch2/table-project/internal/catch_xmlwriter.h/cpp
...
// =================================================================================================
    ...
    struct XmlReporter : public IReporter
    {
        ...
        void test_case_start_impl(const TestCaseData& in) {
            ...
            if(tc != nullptr) { // we have already opened a table-project-tests suite
                ...
                open_ts_tag = true; // first table-project-tests case ==> first table-project-tests suite
            }
        ...
        void test_run_start() override {
            ...
            xml.startElement("main").writeAttribute("binary", binary_name);
            if(opt.no_version == false)
                xml.writeAttribute("version", DOCTEST_VERSION_STR);

            // only the consequential ones (TO#DO: filters)
        ...
        void test_run_end(const TestRunStats& p) override {
            if(tc) // the TestSuite tag - only if there has been at least 1 table-project-tests case
                xml.endElement();
    ...
    struct JUnitReporter : public IReporter
    {
        ...
        void test_run_end(const TestRunStats& p) override {
            ...
            xml.startElement("testsuite").writeAttribute("name", binary_name)
                    ...
                    .writeAttribute("table-project-tests", p.numAsserts);
    ...
    struct ConsoleReporter : public IReporter
    {
        ...
        void printVersion() {
            if(opt.no_version == false)
                s << Color::Cyan << "[main] " << Color::None << "main version is \""
                  << DOCTEST_VERSION_STR << "\"\n";
        }

        void printIntro() {
            printVersion();
            s << Color::Cyan << "[main] " << Color::None
        ...
        void printHelp() {
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "c,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "count                         "
              << Whitespace(sizePrefixDisplay*1) << "prints the number of matching table-project-tests\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ltc, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-table-project-tests-cases               "
              << Whitespace(sizePrefixDisplay*1) << "lists all matching table-project-tests by name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "lts, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-table-project-tests-suites              "
              << Whitespace(sizePrefixDisplay*1) << "lists all matching table-project-tests suites\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "lr,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "list-reporters                "
              << Whitespace(sizePrefixDisplay*1) << "lists all registered reporters\n\n";
            // ================================================================================== << 79
            s << Color::Cyan << "[main] " << Color::None;
            s << "The available <int>/<string> options/filters are:\n\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tc,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "table-project-tests-case=<filters>           "
              << Whitespace(sizePrefixDisplay*1) << "filters     table-project-tests by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tce, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "table-project-tests-case-exclude=<filters>   "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT table-project-tests by their name\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sf,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file=<filters>         "
              << Whitespace(sizePrefixDisplay*1) << "filters     table-project-tests by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "sfe, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "source-file-exclude=<filters> "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT table-project-tests by their file\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ts,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "table-project-tests-suite=<filters>          "
              << Whitespace(sizePrefixDisplay*1) << "filters     table-project-tests by their table-project-tests suite\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "tse, --" DOCTEST_OPTIONS_PREFIX_DISPLAY "table-project-tests-suite-exclude=<filters>  "
              << Whitespace(sizePrefixDisplay*1) << "filters OUT table-project-tests by their table-project-tests suite\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ob,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "order-by=<string>             "
              << Whitespace(sizePrefixDisplay*1) << "how the table-project-tests should be ordered\n";
            s << Whitespace(sizePrefixDisplay*3) << "                                       <string> - [file/suite/name/rand/none]\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "rs,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "rand-seed=<int>               "
              << Whitespace(sizePrefixDisplay*1) << "seed for random ordering\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "f,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "first=<int>                   "
              << Whitespace(sizePrefixDisplay*1) << "the first table-project-tests passing the filters to\n";
            s << Whitespace(sizePrefixDisplay*3) << "                                       execute - for range-based execution\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "l,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "last=<int>                    "
              << Whitespace(sizePrefixDisplay*1) << "the last table-project-tests passing the filters to\n";
            s << Whitespace(sizePrefixDisplay*3) << "                                       execute - for range-based execution\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "s,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "success=<bool>                "
              << Whitespace(sizePrefixDisplay*1) << "table-project successful assertions in output\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "cs,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "case-sensitive=<bool>         "
              << Whitespace(sizePrefixDisplay*1) << "filters being treated as case sensitive\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "e,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "exit=<bool>                   "
              << Whitespace(sizePrefixDisplay*1) << "exits after the table-project-tests finish\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "d,   --" DOCTEST_OPTIONS_PREFIX_DISPLAY "duration=<bool>               "
              << Whitespace(sizePrefixDisplay*1) << "prints the time duration of each table-project-tests\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "nt,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "no-throw=<bool>               "
              << Whitespace(sizePrefixDisplay*1) << "skips exceptions-related assert checks\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ne,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "no-exitcode=<bool>            "
              << Whitespace(sizePrefixDisplay*1) << "returns (or exits) always with success\n";
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "nr,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "no-run=<bool>                 "
              << Whitespace(sizePrefixDisplay*1) << "skips all runtime main operations\n";
            ...
            s << " -" DOCTEST_OPTIONS_PREFIX_DISPLAY "ns,  --" DOCTEST_OPTIONS_PREFIX_DISPLAY "no-skip=<bool>                "
              << Whitespace(sizePrefixDisplay*1) << "don't skip table-project-tests cases marked as skip\n";
        ...
        void printRegisteredReporters() {
            ...
            auto printReporters = [this] (const reporterMap& reporters, const char* type) {
                if(reporters.size()) {
                    s << Color::Cyan << "[main] " << Color::None << "listing all registered " << type << "\n";
        ...
        void list_query_results() {
            ...
            if(opt.count || opt.list_test_cases) {
                s << Color::Cyan << "[main] " << Color::None
                  << "unskipped table-project-tests cases passing the current filters: "
                ...
                s << Color::Cyan << "[main] " << Color::None
                  ...
                  << g_cs->numTestCasesPassingFilters << "\n";
                s << Color::Cyan << "[main] " << Color::None
                  << "table-project-tests suites with unskipped table-project-tests cases passing the current filters: "
        ...
        void report_query(const QueryData& in) override {
            if(opt.version) {
                ...
                if(opt.list_test_cases) {
                    s << Color::Cyan << "[main] " << Color::None
                      << "listing all table-project-tests case names\n";
                    separator_to_stream();
                }
                ...
                s << Color::Cyan << "[main] " << Color::None << "listing all table-project-tests suites\n";
                separator_to_stream();
        ...
        void test_run_end(const TestRunStats& p) override {
            ...
            s << Color::Cyan << "[main] " << Color::None << "table-project-tests cases: " << std::setw(totwidth)
            ...
            s << "\n";
            s << Color::Cyan << "[main] " << Color::None << "assertions: " << std::setw(totwidth)
              << p.numAsserts << " | "
        ...
        void test_case_end(const CurrentTestCaseStats& st) override {
            ...
            // log the preamble of the table-project-tests case only if there is something
        ...
        void test_case_exception(const TestCaseException& e) override {
            ...
            successOrFailColoredStringToStream(false, e.is_crash ? assertType::is_require :
                                                                   assertType::is_check);
            s << Color::Red << (e.is_crash ? "table-project-tests case CRASHED: " : "table-project-tests case THREW exception: ")
...
#define DOCTEST_DEBUG_OUTPUT_REPORTER_OVERRIDE(func, type, arg)                                    \
    ...
    bool parseIntOption(int argc, const char* const* argv, const char* pattern, optionType type,
        ...
        if(type == 0) {
            ...
            // integer
            // TO#DO: change this to use std::stoi or something else! currently it uses undefined behavior - assumes '0' on failed parse...
...
void Context::parseArgs(int argc, const char* const* argv, bool withDefaults) {
    ...
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "table-project-tests-suite=",         p->filters[2]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ts=",                 p->filters[2]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "table-project-tests-suite-exclude=", p->filters[3]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "tse=",                p->filters[3]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "table-project-tests-case=",          p->filters[4]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "tc=",                 p->filters[4]);
    parseCommaSepArgs(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "table-project-tests-case-exclude=",  p->filters[5]);
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                                        \
    ...
    if(parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "list-table-project-tests-cases") ||
       parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "ltc")) {
        p->list_test_cases = true;
        p->exit            = true;
    }
    if(parseFlag(argc, argv, DOCTEST_CONFIG_OPTIONS_PREFIX "list-table-project-tests-suites") ||
...
// the main function that does all the filtering and table-project-tests running
int Context::run() {
    using namespace detail;
    ...
    // TO#DO: check if there is nothing in reporters_currently_used
    ...
    if(!testArray.empty()) {
        if(p->order_by.compare("file", true) == 0) {
            ...
            // means no sorting - beneficial for death table-project-tests which call into the executable
            // with a specific table-project-tests case in mind - we don't want to slow down the startup times
        }
    }
    ...
    for(auto& curr : testArray) {
        ...
        // skip the table-project-tests if it is not in the execution range
        ...
        // do not execute the table-project-tests if we are to only count the number of filter passing table-project-tests
        if(p->count)
            continue;

        // print the name of the table-project-tests and don't execute it
        if(p->list_test_cases) {
            queryResults.push_back(&tc);
            continue;
        }

        // print the name of the table-project-tests suite if not done already and don't execute it
        ...
        // execute the table-project-tests if it passes all the filtering
        {
            p->currentTest = &tc;
...
DOCTEST_MSVC_SUPPRESS_WARNING_WITH_PUSH(4101) // unreferenced local variable
                    ...
                    // execute the table-project-tests
                    tc.m_test();
                    fatalConditionHandler.reset();
DOCTEST_MSVC_SUPPRESS_WARNING_POP
#ifndef DOCTEST_CONFIG_NO_EXCEPTIONS
            ...
            // stop executing table-project-tests if enough assertions have failed
